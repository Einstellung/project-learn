...
'''Uniqueness:'''
# by adding a unique constant at each round, the algorithm ensures that even if the input data or intermediate states are similar,
# the addition of the round constants will make the paths diverge, leading to different outcomes.
'''Security:'''
# Round constants help in thwarting attacks by making it harder for attackers to reverse-engineer the hash function or find collisions
'''Predifined values:'''
# These constants are not random; they are carefully chosen and fixed as part of the hash function design.
...
def calc_round_constants(t, full_round, partial_round, p, field_p, alpha, prime_bit_len):
    ...
    rc_field = []
    # Discard first 160 output bits: (common practice to avoid initial state biases affecting the output sequence)
    ...
    while len(rc_field) < rc_number:
        # use self-shrinking mode to add non-linearity, enhance security
...
# It refers to a matrix that achieves maximum diffusion in the in the minimum number of steps
# Diffusion means that changing one bit of the input affects many bits of the output, making it harder
# to find correlations between the input and the output

# In the context of cryptography functions, an MDS matrix is used to ensure that each output bit is 
# influenced by every input bit, achieving the so-called "avalanche effect".

# MDS matrices are known for their property of maximizing the minimum Hamming distance between different output vectors, which in cryptographic applications helps in achieving high levels of security against attacks.
def mds_matrix_generator(field_p, t):
    ...
    :rtype:
    """
    # change each element will influnce whole matrix
    ...
    for i in range(t):
        for j in range(t):
            # inverse operation is sensitive; small change in the input(before taking the inverse) lead to
            # significant and widely spread changes in the output
