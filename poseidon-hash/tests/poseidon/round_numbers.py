...
# the term "round numbers" refers to the total number of iterations or cycles that the algorithm goes through during
# its execution. Each round will typically involve a series of operations substitution, permutations, mixing...

# "round numbers" are the number of full and partial rounds
# they are used to calculate the number of S-boxes which equal t * full_round + partial_round

# calculate the optimal number of full and partial rounds for Poseidon hash function to achieve a desire security level.
def calc_round_numbers(prime_bit_len, security_level, t, alpha, security_margin):

    
    # "t":
    # 't' is analogous to the number of bowls you have available to mix ingredients in a kitchen, rather than the size of single bowl

    # A large number of bowls(a larger "t") means the hash function can handle a larger internal state per round, which can improve
    # secruity but may also increase the computational cost of the hash function.

    # Each bowl's size is determined by the size of the field over which the hash function operates, not by "t". "t" just tells you
    # how many of these bowls you have. So, if "t" is large, you're not reducing the size of each bowl; you're simply increasing the
    # number of bowls you have to mix your ingredients in. 
    """
