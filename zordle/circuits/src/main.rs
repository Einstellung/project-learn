use halo2_proofs::{pasta::group::ff::PrimeField, circuit::*, plonk::*, poly::Rotation};
...
fn verify_play(final_word: String) {
    ...
    /* fs::read is running time to read file, and just can read file from ./src... */
    // let proof_vec = fs::read("proof.bin").expect("Failed to read proof");
    // let proof = &proof_vec[..];
...
fn write_params() {
    let mut params_file = File::create("params.bin").expect("cannot create params.bin");
    let params: Params<EqAffine> = Params::new(K);
    params.write(&mut params_file).expect("cannot write params.bin");
    println!("Successfully wrote params to params.bin");
}
