...
问：为什么$\vec{f}'=(9,9,9,9,9,9,9,9)$ ，也会满足上面的等式，明明 $f'(X)-t'(X)$ 是不相等的？

答：确实是不相等的。这个没有问题。但是实际要比较的等式是 $(f'(X)-f'(\omega^{-1}\cdot X))\cdot (f'(X)-t'(X)) = 0$ 其中 $(f'(X)-f'(\omega^{-1}\cdot X))$ 是永远等于`0`的，所以不管 $f'(X)-t'(X)$ 是否等于`0`，整个式子都是等于`0`的，相当于部分约束无效了。所以需要额外加一个约束。

问：既然需要额外加一个约束项，为什么不能直接写成 $(f'(X)-f'(\omega^{-1}\cdot X)) + \alpha(f'(X)-t'(X)) = 0$ 加法运算的形式就规避了乘积的问题，少写一个后面特定对 $f'(x)$ 和 $t'(x)$ 的约束项？

答：因为这是一个或的关系，写成乘积的形式可以非常好的表达或这个形式。 $f'(x)$ 既可以等于 $f'(w^{-1} \cdot x)$ 也可能等于 $t'(x)$ 不能写成加和，加法运算就是and关系了。
...
预处理：Prover 和 Verifier 构造 $t(X)$，该多项式是大家公有的，此处 $t(X)$ 已经要生成commitment了
...
如果向量 $\vec{s}$ 满足 $\{s_i\}\subseteq_{set}\{t_i\}$，并且 $\{f_i\}\cup\{t_i\}=\_{multiset} \{s_i\}$，那么就可以证明 $\{f_i\}\subseteq_{set} \{t_i\}$。（set要求集合中元素不重复，multiset集合可以出现重复元素）
...
&\quad + \alpha^4 [\bar{z'}(w\zeta) g_4(X) - z'(X) g_3(\zeta)] \\
...
$$
\begin{align*}
g_3(\zeta) &= \Big((1+\beta_2)(\bar{f}(\zeta)+\gamma_2)\Big)\cdot\Big(\bar{t}(\zeta)+\beta_2\cdot \bar{t}(\omega\cdot \zeta)+\gamma_2(1+\beta_2)\Big) \\
\\
g_4(X) &= \Big({\color{blue}s^{even}(X)}+\beta_2\cdot {\color{blue}\bar{s}^{odd}(\zeta)}+\gamma_2(1+\beta_2)\Big)\cdot\Big({\color{blue}\bar{s}^{odd}(\zeta)}+\beta_2\cdot {\color{blue}\bar{s}^{even}(\omega\cdot \zeta)}+\gamma_2(1+\beta_2)\Big)
\end{align*}
$$
