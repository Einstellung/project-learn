...
class VirtualMachine:
    ...
    # transform brainfuck code(char) into machine code(int, or we can say instruction code)
    def compile(brainfuck_code):
        ...
        zero = field.zero()
        one = field.one()
        # this function is used to transform a char into unicode integer(in finite field)
        ...
        for symbol in brainfuck_code:
            ...
            if symbol == '[':
                ...
                program += [zero]
                # express "[" position in the program
                stack += [len(program) - 1]
            elif symbol == ']':
                # record loop's end
