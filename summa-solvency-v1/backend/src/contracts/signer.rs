...
pub struct SummaSigner {
    /* By locking around the submission of the transaction, the code ensure that no other part of the code
       the code that also use the `nonce_lock` can run at the same time. This prevents two transaction from
       being sent with the same nonce.
     */
...
impl SummaSigner {
    ...
    pub async fn submit_proof_of_address_ownership(
        ...
        /* acquires the lock asynchronously. If other part of the code has already acquired the lock,
        ...
        /* releases the lock by dropping the `lock_guard` */
        drop(lock_guard);
        Ok(())
    }
