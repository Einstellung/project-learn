...
contract Summa is Ownable {
     ...
     */
    // TODO: what if balnk space? if signature and message is blank space?
    function submitProofOfAddressOwnership(
        ...
        for (uint i = 0; i < _addressOwnershipProofs.length; i++) {
            /* This function takes a variable number of arguments and performs tight packing of the data before encoding.
               Tight packing means that the encoded data is concatenated together without padding.
               Since strings in Solidity are of dynamic size and `keccak256` expects a bytes argument, `abi.encodePacked`
               effectively converts the string to a bytes representation  
            */
    ...
    function verifyInclusionProof(
        ...
        /* when you call `inclusionVerifier.verifyProof` you are actually calling the implemented function 
           from the `InclusionVerifer` contract whichever contract was passed in that implements the `IVerfier` interface.
           in contract we just send implement IVerifier actaully is "InclusionVerifier.sol" address, so we can use here inclusionVerifier.verifyProof beacuse inclusionVerifier is from other contract, we just call other contract function.
         */
