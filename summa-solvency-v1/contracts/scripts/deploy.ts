...
async function main() {
  /* `src/InclusionVerifier.sol:Verifier` this string is not just the name of contract; it's path and contract combined.
     This syntax is used when a Solidity file contains multiple contracts, and you need to specify which contract within
     the file you want to deploy.
     
     src/InclusionVerifier.sol - This is the path to the Solidity file relative to the sources directory of the Hardhat project.

     :Verifier - This specifies that within InclusionVerifier.sol, we want to deploy the contract named Verifier.
   */
